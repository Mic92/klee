...
namespace {
  ...
  cl::opt<klee::loader::LibcType>
  Libc("libc",
       cl::desc("Choose libc version (none by default)."),
       cl::values(clEnumValN(klee::loader::NoLibc, "none", "Don't link in a libc"),
                  clEnumValN(klee::loader::KleeLibc, "klee", "Link in klee libc"),
		  clEnumValN(klee::loader::UcLibc, "uclibc", "Link in uclibc (adapted for klee)")
		  KLEE_LLVM_CL_VAL_END),
       cl::init(klee::loader::NoLibc));
...
void externalsAndGlobalsCheck(const Module *m) {
  ...
  switch (Libc.getValue()) {
  case klee::loader::LibcType::KleeLibc:
    dontCare.insert(dontCareKlee, dontCareKlee+NELEMS(dontCareKlee));
    break;
  case klee::loader::LibcType::UcLibc:
    dontCare.insert(dontCareUclibc,
                    dontCareUclibc+NELEMS(dontCareUclibc));
    break;
  case klee::loader::LibcType::NoLibc: /* silence compiler warning */
    break;
  }
...
int main(int argc, char **argv, char **envp) {
  ...
  std::string libraryPath = KleeHandler::getRunTimeLibraryPath(argv[0]);
  Interpreter::ModuleOptions Opts(libraryPath.c_str(), EntryPoint,
                                  /*Optimize=*/OptimizeModule,
                                  /*CheckDivZero=*/CheckDivZero,
                                  /*CheckOvershift=*/CheckOvershift);

  klee::loader::LoadOptions loadOptions = { };
  loadOptions.libc = Libc.getValue();
  loadOptions.enablePOSIXRuntime = WithPOSIXRuntime;
  loadOptions.linkLibraries = LinkLibraries;

  // Load the bytecode...
  Module *mainModule = klee::loader::loadInputFile(InputFile, EntryPoint, libraryPath, loadOptions);
  ...
  theInterpreter = Interpreter::create(mainModule->getContext(), IOpts, handler);
