...
private:
  ...
  /// A list of partial ordered branch samples the symbolic execution tries to follow,
  /// when running in
  const std::vector<BranchSample> *branchSamples;
...
public:
  ...
  virtual void setBranchSamples(const std::vector<BranchSample> *samples) {
    branchSamples = samples;
  }
